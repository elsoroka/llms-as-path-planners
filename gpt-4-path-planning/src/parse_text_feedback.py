"""
Convert text_feedback.py JSONL output into the format expected by evaluate.py.

Direction tokens (up, down, left, right) are extracted directly from each
attempt's response; no expansion step is needed (unlike parse_code_form.py).
...
Supports both output formats:
  ...
  - Single-response: record has a top-level "response" field.
...
def main():
    ...
    for r in records:
        ...
        out.append({
            "english":        r['naive_representation'],
