...
# Import check_stl_math from the PPNL ICL directory.
...
ONE_SHOT_FORMULA = "G(not(x >= 5 and x <= 8 and y >= 3 and y <= 6)) and F(x == 12 and y == 5)"
...
def build_prompt(sample: dict) -> str:
    """Build the STL math generation one-shot prompt for a single gpt-4 task."""
    return (
        STL_PROMPT + "\n"
        "For example:\n"
        "Description in code:\n"
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="STL math-form generation for gpt-4-path-planning datasets."
    )
